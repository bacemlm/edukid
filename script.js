...
let timer = null;
let quizCategory = configContainer.querySelector(".category-option.active").textContent;
let numberOfQuestions = parseInt(configContainer.querySelector(".question-option.active").textContent);
...
const showQuizResult = () => {
  ...
  const resultText = `لقد أجبت على<b>${correctAnswersCount}</b> من ${numberOfQuestions}</b> أسئلة صحيحة`;
...
const highlightCorrectAnswer = () => {
  ...
  const iconHTML = `<span class="material-symbols-rounded"></span>`;
...
const handleAnswer = (option, answerIndex) => {
  ...
  const iconHTML = `<span class="material-symbols-rounded"> ${isCorrect ? "" : ""} </span>`;
...
const renderQuestion = () => {
    ...
    // Question image
    const questionImage = quizContainer.querySelector(".question-image");
    ...
    // Question status
    questionStatus.innerHTML = `<b>${questionsIndexHistory.length}</b> من <b>${numberOfQuestions}</b> أسئلة`;
